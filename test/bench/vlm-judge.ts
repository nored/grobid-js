...
//
// Inputs:
//   fixtures/external-corpus/vlm-truth/paper_NNN.json — VLM-extracted truth
//   fixtures/external-corpus/upstream/paper_NNN.tei.xml — Java GROBID output
//   fixtures/external-corpus/grobid-js/paper_NNN.tei.xml — our output
//
// Output: test/bench/vlm-judge-report.md
//
// Re-run with: `npx tsx test/bench/vlm-judge.ts`
...
const reportPath = path.join(here, `vlm-judge-report${reportSuffix}.md`);
...
function scoreAffiliations(truthAff: string[], teiAff: string[]): number {
  if (truthAff.length === 0) return 1;
...
function scoreAbstract(truthPrefix: string, teiAbstract: string): number {
  // Sentinel: if the truth file only carries the keyword "ABSTRACT" then
  // there was nothing visible on page 1 (cover sheets etc.). Skip.
...
export function writeReport(): void {
  ...
  out.push(
    `Both TEIs are scored against that truth. ${rows.length} papers scored. Re-run: \`npx tsx test/bench/vlm-judge.ts\`.`,
  );
  out.push("");
...
// Allow `npx tsx test/bench/vlm-judge.ts` to drive the report directly.
